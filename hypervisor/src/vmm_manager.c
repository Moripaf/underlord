...
static int verify_vmm_image(void)
{
    ...
    if (cpio_get_file(_cpio_archive, archive_size, VMM_IMAGE_NAME, &size) == NULL ||
        size == 0) {
        underlord_hlog(UNDERLORD_LOG_ERROR, "CPIO image '%s' missing",
                       VMM_IMAGE_NAME);
        return -1;
    }
    return 0;
...
int vmm_instance_start(hypervisor_context_t *context, vmm_instance_t *instance)
{
    ...
    if (sel4utils_configure_process_custom(&instance->process, &context->vka,
        ...
        underlord_hlog(UNDERLORD_LOG_ERROR,
                       "VMM instance %u configuration failed", instance->id);
        return -1;
    }
    ...
    if (vka_alloc_endpoint(&context->vka, &instance->control_endpoint) != 0) {
        underlord_hlog(UNDERLORD_LOG_ERROR,
                       "VMM instance %u control endpoint allocation failed",
                       instance->id);
        return -1;
    }
    ...
    if (child_slot != SEL4UTILS_FIRST_FREE) {
        underlord_hlog(UNDERLORD_LOG_ERROR,
                       "VMM instance %u control capability install failed",
                       instance->id);
        return -1;
    }

    underlord_hlog(UNDERLORD_LOG_INFO, "VMM instance %u created", instance->id);
    if (sel4utils_spawn_process_v(&instance->process, &context->vka,
                                  &context->vspace, 0, NULL, 1) != 0) {
        underlord_hlog(UNDERLORD_LOG_ERROR,
                       "VMM instance %u start failed", instance->id);
        return -1;
    }
    instance->state = VMM_RUNNING;
    underlord_hlog(UNDERLORD_LOG_INFO, "VMM instance %u started", instance->id);
    return 0;
}
...
void vmm_instance_supervise(vmm_instance_t *instance)
{
    ...
    underlord_hlog(UNDERLORD_LOG_ERROR,
                   "VMM instance %u faulted (badge=%lu, label=%lu)",
